//
// Warning : DO NOT EDIT ! file generated automatically
// TRUST_NO_INDENT
//
#include <verifie_pere.h>
#include <Champ_Don_Face_lu.h>
#include <Champ_Face_VDF.h>
#include <Champ_Fonc_Face_VDF.h>
#include <Champ_Fonc_P0_VDF.h>
#include <Champ_Fonc_P1_VDF.h>
#include <Champ_Fonc_Q1_VDF.h>
#include <Champ_Fonc_Tabule_P0_VDF.h>
#include <Champ_Ostwald_VDF.h>
#include <Champ_P0_VDF.h>
#include <Champ_front_debit_QC.h>
#include <Champ_front_debit_QC_fonc_t.h>
#include <Champ_front_fonc_gradient_VDF.h>
#include <Champ_som_lu_VDF.h>
#include <Champ_val_tot_sur_vol_VDF.h>
#include <Correlation_Vec_Sca_VDF.h>
#include <Courant_maille_Champ_Face.h>
#include <Critere_Q_Champ_Face.h>
#include <Reynolds_maille_Champ_Face.h>
#include <Rotationnel_Champ_Face.h>
#include <Taux_cisaillement_P0_VDF.h>
#include <Y_plus_Champ_Face.h>
#include <grad_U_Champ_Face.h>
void instancie_src_VDF_Champs() {
Cerr << "src_VDF_Champs" << finl;
Champ_Don_Face_lu inst1;verifie_pere(inst1);
Champ_Face_VDF inst2;verifie_pere(inst2);
Champ_Fonc_Face_VDF inst3;verifie_pere(inst3);
Champ_Fonc_P0_VDF inst4;verifie_pere(inst4);
Champ_Fonc_P1_VDF inst5;verifie_pere(inst5);
Champ_Fonc_Q1_VDF inst6;verifie_pere(inst6);
Champ_Fonc_Tabule_P0_VDF inst7;verifie_pere(inst7);
Champ_Ostwald_VDF inst8;verifie_pere(inst8);
Champ_P0_VDF inst9;verifie_pere(inst9);
Champ_front_debit_QC inst10;verifie_pere(inst10);
Champ_front_debit_QC_fonc_t inst11;verifie_pere(inst11);
Champ_front_fonc_gradient_VDF inst12;verifie_pere(inst12);
Champ_som_lu_VDF inst13;verifie_pere(inst13);
Champ_val_tot_sur_vol_VDF inst14;verifie_pere(inst14);
Correlation_Vec_Sca_VDF inst15;verifie_pere(inst15);
Courant_maille_Champ_Face inst16;verifie_pere(inst16);
Critere_Q_Champ_Face inst17;verifie_pere(inst17);
Reynolds_maille_Champ_Face inst18;verifie_pere(inst18);
Rotationnel_Champ_Face inst19;verifie_pere(inst19);
Taux_cisaillement_P0_VDF inst20;verifie_pere(inst20);
Y_plus_Champ_Face inst21;verifie_pere(inst21);
grad_U_Champ_Face inst22;verifie_pere(inst22);
}
